...
TileTextureSet_t* TileTextureSet_CreateFromGameData( MemArena_t* memArena, GameData_t* gameData )
{
   ...
   fileOffset = gameData->offsets.tileTextureSet;
   if ( (i32)( fileOffset + sizeof( TileTextureSet_t ) ) > gameData->file->size )
   {
   ...
   Platform_FileSeek( gameData->file, fileOffset, 0 );
   Platform_ReadFileBytes( gameData->file, (u8*)( tileTextureSet ), sizeof( TileTextureSet_t ) );
   ...
   if ( tileTextureSet->count > 0 )
   {
      ...
      if ( ( textureDataOffset + textureDataSize ) > gameData->file->size )
      {
      ...
      Platform_FileSeek( gameData->file, textureDataOffset, 0 );
      Platform_ReadFileBytes( gameData->file, (u8*)( tileTextureSet->textures ), textureDataSize );
   }
